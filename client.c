	...
	while (1)
	{
		...
		if (receive > 0)
		{
			...
			if (flag_code)
			{
				...
				if (strcmp(json_object_get_string(response), "GET_CHAT") == 0)
				{ // GET CHAT LISTA DE MENSAJES
...
void sendv_msg()
{
	...
	while (1)
	{
		...
		else if (strcmp(msg, "user_msg") == 0)
		{
			...
			// printf("here 3");
			str_parser(msg, 2000);
			str_parser(option, 2000);	// a quien mensaje
		...
		else if (strcmp(msg, "change-status") == 0)
		{
			...
			// printf("here 3");
			str_parser(msg, 2000);	   // opcion
			str_parser(option, 2000); // que cambio hacer
			char choice[1];
